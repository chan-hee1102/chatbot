...
export default function ChatWindow({ conversationId, title, onAfterFirstMessage }: ChatWindowProps) {
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 h-full">
      <header className="border-b border-gray-800 px-4 sm:px-6 py-3 flex items-center min-h-[3.25rem]">
        <h2 className="text-base font-medium text-gray-200 truncate">{title}</h2>
      </header>
        ...
        <div className="max-w-3xl mx-auto">
          {loading ? (
            ...
            <div className="text-center text-gray-500 py-16">
              <p className="text-lg mb-1">새 대화를 시작하세요</p>
              <p className="text-sm">아래에 메시지를 입력하세요.</p>
            </div>
          ) : (
            <>
              {messages.map((m) => (
          ...
          {error && (
            <div className="my-3 text-sm text-red-400 bg-red-950/40 border border-red-900/60 rounded-lg px-3 py-2">
              {error}
            </div>
          )}
        </div>
